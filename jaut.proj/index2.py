...
def submit():
    ...
    if type.get() != "" and name.get() != "" and price.get() != "":
        ...
        messagebox.showinfo("error", "tukša ievade")
    conn.commit()
    type.insert(0, "")
    name.insert(0, "")
    price.insert(0, "")
...
def delete():
    ...
    if treeIndex:
        ...
        messagebox.showinfo("Error","atzīmē dzēšamo vērtību")
...
def update():
    ...
    if not edit:
        ...
        if treeIndex:
            ...
            messagebox.showinfo('error',"tu i tups")
    else:
        if type.get() != "" and name.get() != "" and price.get() != "":
...
def printout():
    ...
    if treeIndex:
        ...
        cursor.execute("SELECT * FROM datasheet WHERE id = "+ str(edit_id))
        part = cursor.fetchone()
        with open("printout"+ str(part[0]),"w") as f:
            f.write("personala datora sastavdala \n")
            f.write('veids: ' + part[1] )
            f.write("\nmodelis: "+ part[2])
            f.write("\ncena: " + str(part[3]))
    else:
        messagebox.showinfo("error","ludzu, atzīmē vēlamo printējamo")
